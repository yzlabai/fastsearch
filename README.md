...
- 🖼️ **Figures as retrievable chunks** — embedded images (PDF + DOCX + PPTX) become `image` chunks: caption + surrounding context fill the searchable `text`, while `file`/base64 + page + bbox let RAG render & cite them. Caption binds the adjacent in-document "Figure N" line for free; `--vlm-describe` upgrades it to a neural description
