...
|---|---|---|---|
| vs OpenDataLoader（确定性同类，15 份） | **0.764** | **0.627** | 0.098 |
| vs Docling（神经管线，13 份） | **0.833** | **0.645** | 0.187 |
...
| [`docparse-ocr`](crates/docparse-ocr) | ONNX 内嵌 OCR enhancer：PP-OCRv4 det+rec 经 `tract` 纯 Rust 推理（DBNet 后处理/CTC 解码自研），实现 `core::enhance::Enhancer` | tract-onnx, zune-jpeg |
...
- [ ] **Phase 4（进行中规划）**：补齐 Docling 占优轴——版面/表结构 ONNX enhancer（难页路由）、格式平齐（XLSX/PPTX/邮件/字幕/图片即文档/LaTeX 等）、区域级 OCR/Form 流、语义增强面（代码块/公式/图片/图表/整页 VLM——VLM 经 OpenAI 兼容服务接入：vLLM/Ollama 等）、LangChain/LlamaIndex 接入、千份语料压测。见 [迭代计划](docs/plans/closing-docling-gaps.md)。
