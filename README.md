...
- [ ] **Phase 4（进行中规划）**：补齐 Docling 占优轴——版面/表结构 ONNX enhancer（难页路由）、XLSX/PPTX、区域级 OCR/Form 流、语义增强面（代码块/公式/图片/图表/整页 VLM——VLM 经 OpenAI 兼容服务接入：vLLM/Ollama 等）、LangChain/LlamaIndex 接入、千份语料压测。见 [迭代计划](docs/plans/closing-docling-gaps.md)。
