...
- **重依赖 opt-in、诚实记账**：未落地的重件——真语义嵌入(Candle/ort)、HNSW+量化、CDC pgoutput 线缆层、k1/b 自定义 BM25、MCP——文档里如实标 `下一迭代`，当前用确定性基线占位（HashEmbedder / MemVectorIndex 暴力余弦 / LexicalOverlapReranker / sync 仅 apply 核心）。**别把基线说成语义/生产级**。
