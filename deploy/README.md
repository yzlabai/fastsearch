...
- **容器镜像 build / K8s 实跑：`待运行验证`** —— 编写当时 Docker Hub registry 不可达（拉
  `rust:1.85-slim` 基镜 EOF），未能本环境构建镜像；K8s 部署需集群。registry/集群可用后按上文
  `docker compose up --build` 与 `kubectl apply -f deploy/cloudnativepg.yaml` 验证。
