...
- `search(collection, query, {mode,top_k,filter,vector,highlight,fusion,query_image,embedder,candidates,rerank,auto_merge,collapse,search_after,facets,explain})`：Python 回 `hits[]`（分面用 `search_with_facets` 回 `{hits,facets}`）；TS 回 `{hits,facets}`（`searchHits` 只要数组）。**collection 作用域强制注入**为 Eq 过滤、与用户 filter `and` 合并（M23）。
