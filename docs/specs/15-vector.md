...
- ✅ **HNSW 随机门禁稳定化（FS-001）**（2026-08-30）：`hnsw_rs 0.3.4` 未暴露 seed，因此不伪造单图确定性，也不为测试抖动改生产图参数。门禁拆成：① 绝对质量——固定 1,500×32 语料、50 查询对照 `MemVectorIndex` ground truth，500 个 top-k 判定的聚合 `recall@10 >= 0.90`；② ef 相对性质——同图上 `over_fetch=1`，从测试低档 `ef=10` 逐查询覆盖到 `ef=512`，必须产生严格召回增益；故意忽略 override 的突变会变红。阈值未下调；修复前精确用例 19/20，修复后两门禁各独立构图 100/100 全绿。详见 [devlog](../devlog/2026-08-30-FS-001-HNSW随机门禁稳定化.md)。
...
- ✅ **精排 sidecar 洞回收 + 压实（Step 3）**（2026-07-22）：**修 reload 泄漏**——`load`/`ensure_sidecar` 经 `slot_state()` 重建 `free`（`[0,next)` 未占的洞），reload 后新 upsert 复用洞、文件不超 `max-ever-live·stride`（此前 `free` 清空 → churn 型删增无界增长，是 bug）。+ `TurboVectorIndex::compact()`：活条目 f32 重写成紧凑连续 slot（原子 写临时→rename→重开 + 重指 slot），文件缩到 `live·stride`——供永久删除后回收磁盘（churn 由 free 控、compact 处理永久缩量，参照 HNSW compact；亦可 rebuild-from-PG 回收）。mmap/`read_at` 未做（候选批小、无实测瓶颈，避 Speculative）。+2 测（reload 复用洞、compact 缩量 12→6 无脏读）。workspace 322 绿。
