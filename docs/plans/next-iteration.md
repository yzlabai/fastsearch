...
> **进度快照（2026-06-10）**：N1 ✅、N2 ✅、N3 ✅（P4 路线：ONNX 内嵌 OCR）、N4 大部 ✅、N5（a+b）✅、N6 已并入 N3 ✅——**本计划的近期里程碑全部收官**；剩 N5c 画像（远期）与确定性天花板项。记分牌：vs ODL NID 0.764/MHS 0.627/TEDS 0.098；vs Docling 0.833/0.645/0.187（clean LTR 0.94–1.00 达同台水平）。以下原文保留为立项时的判断。
...
### N5 · 安全预检与复杂度画像 — *模块 9* · ✅ 全部完成（a+b+c，[plan](n5-security-precheck.md)）
...
- [x] **复杂度画像**（N5c，2026-06-10）：`quality::PageProfile`（digital/scanned/mixed/empty + 图覆盖/表密度/enhanced 占比）；CLI `--profile`、MCP 信封 `profile`。旋转信号留 TODO。
