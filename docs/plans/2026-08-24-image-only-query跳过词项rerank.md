...
**这是 H1 的第三条链路，不是孤立新 bug**（2026-08-24 复核时理清）：
[2026-07-05 代码审查报告](../reviews/2026-07-05-代码审查报告.md) 的 H1「请求 rerank 反而摧毁排序」
已把"全 0 分 → 全同分 → gid 序压平真实排名"定为高危，但只列了两条链路——H1-A（候选文本为空串）
与 H1-B（CJK 整句成单 token），二者均已修。本条是 **H1-C：query 本身为空**。

它能从那轮 40+ 处发现的深度审查里活下来，是因为落在两个 finding 的**交集**上：同轮 **M7** 修的是
"空 query + query_image 在 Hybrid 下 keyword 路退化成 match-all"——**召回侧**的空 query；
H1 修的是**非空 query** 下的 rerank 打分。"空 query × rerank"两边都不覆盖。
判据上的教训：按"输入维度的组合"而不是按"模块"去枚举边界，否则交集永远是盲区。
