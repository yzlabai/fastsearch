...
| **M2** | 机器可读 + 报告增强 | I4 `--progress=json` 事件流、I5 报告显示相对路径、I6 报告小料 |
| **M3** | 吞吐 | I7 文件级并行 `--jobs`(需先 spike) |
...
**问题**:`write_output` 的 `rel` 当前来自 `strip_prefix(base)`(必相对、无 `..`)或显式文件的 `file_name()`,实测无逃逸。但 `file_name()` 在病态路径(以 `/`、`..` 结尾)会回退到 `as_os_str()`,理论上能拼出绝对/越界路径(`dir.join("/x")` 在 unix 会**替换** `dir`)。这些路径过不了 `is_file()`,当前不可达,但属脆弱点。

**方案**:`rel` 落盘前断言"相对且无 `..` 组件",否则退化为纯 `file_name()` 或拒绝。一个 `sanitize_rel(rel) -> PathBuf` 小工具 + 单测。
**风险**:低。**effort:S。**

### I4 · `--progress=json` 机器可读事件流（M2，P1，原 FR7）

**问题**:CI / 上层封装拿不到进度——人读表格与进度条无法解析。

**方案**:在 stderr 输出 JSON-lines 事件(`phase_start`/`phase_end`/`file_done`/`summary`),schema 与批量报告 + REST `x-docparse-ms` 对齐。开关:`ProgressMode` 扩 `Json` 或新加 `--progress-format human|json`。进度条与 JSON 互斥(JSON 时不画条)。
**影响面**:[progress.rs](../../crates/docparse-cli/src/progress.rs) 加事件发射;batch/single 调用点喂事件。
**风险**:低。**effort:S–M。**

### I5 · 报告显示相对路径,消歧递归同名（M2，P2，review 暴露）

**问题**:递归批量里 `alpha/paper.pdf`、`beta/paper.pdf` 在人读表格里都显示为 `paper.pdf`,两行看着一样(JSON/CSV 报告带全 `path`,无歧义)。

**方案**:`FileStat` 携 `rel`(或显示路径),批量/递归时表格显示 `rel` 而非裸名。
**风险**:低。**effort:S。**
