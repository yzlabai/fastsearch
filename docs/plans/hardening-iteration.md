...
>
> **✅ 收官状态(2026-06-12)**:7 个里程碑全部下结论。功能落地 H1(CCITT/JBIG2)、H2(/Rotate+cls 旋转校正)、H4(双栏左列重排)、H5(精确 TEDS)、H7 之 HTML charset/CMYK JPEG/MediaBox 继承;**负结论**(均做到能下定论)H3(读序异常分不可分→关闭确定性自动路由,信号留诊断)、H6(建了完整背景模型后确证:同色文本检测与"不渲染"身份根本冲突,架构边界不做);**阻塞留待** H7 之转写去重(需 unirec 模型验证)、v5-mobile(需离线 paddle2onnx)。8 提交,142 单测绿,clippy 0,双记分牌全程基线(NID 0.792/0.822)。收官总结见文末。
...
## 4. Phase 5 收官总结(2026-06-12)

| 里程碑 | 结论 | 提交 | devlog |
|---|---|---|---|
| H1 CCITT/JBIG2 扫描解码 | ✅ 传真/JBIG2 扫描件 OCR 出文字;依赖本就传递在树,零供应链增量 | a32f2f5 | h1-ccitt-jbig2 |
| H2 旋转校正(/Rotate + cls) | ✅ 四方向自动转正,docling 三件相似度 1.000×3;/Rotate 烘入基 CTM、归一化页语义 | a32f2f5 | h2-cls-rotation |
| H5 精确 TEDS(Zhang-Shasha) | ✅ 树编辑距离上线,span 被正确奖励(ODL 0.419→0.477、Docling 0.474→0.526) | 338b50f | h5-exact-teds |
| H3 读序异常分 + 路由收口 | ⊘ 第 4 判据仍不可分→**关闭确定性自动路由**;异常分作诊断信号进 profile | 132e292 | h3-reading-order-anomaly |
| H4 双栏左列 fill_x | ✅ 列感知 per-line fill_x,左列正文聚段,单栏字节级零回归 | 5b8f223 | h4-two-column-reflow |
| H6 同色文本检测 | ✗ **架构边界**:建了完整背景模型(device块/图/渐变/form)仍无法零误杀,本质需渲染→不做 | 8eb4c53 / 001127f | h6-*(两篇) |
| H7 打磨篮子 | 🟡 HTML charset + CMYK JPEG + MediaBox 继承落地;转写去重/v5-mobile 阻塞留待 | b4f534f | h7-polish |

**沉淀的教训**(已在各 devlog 详记,择要):
1. **判据先探针后落码**(H3):Python 探针 10 分钟出可分性结论,避免又一轮"写进 Rust→回退→留死代码"。
2. **负结论要建到能下定论的程度**(H6):前一轮"白底假设误杀"只够说"需背景模型";真把模型建出来才看清是"架构边界"而非"工作量"——区分"没做对"与"做不到"才算结论。
3. **安全特性更要先看记分牌**(H6):合成样例 + bialetti 都过了,normal_4pages 韩文封面(form+渐变反白标题)才暴露误杀;安全特性验收语料必须含设计页/封面/多语种。
4. **零回归靠"退化为旧路径"**(H4):列感知做成"无 gutter 时逐行恒等旧单值",单栏字节级不变——高风险核心改动的安全垫是新逻辑在常见情形与旧逻辑数学等价。
5. **不渲染是边界也是身份**(H6/CMYK):同色检测、CMYK 正确转色都触到"需要像素"的边界;确定性核心如实交出信号/占位,把渲染留给可插拔域。

**遗留(下阶段候选)**:H1/H2 自审遗留的归一化页×下游栅格增强器错帧、导出忽略 `turns`、转写/layout 无方向校正、走查器统一(见 H7 上方"自审遗留"段);H7 转写去重、v5-mobile;按需池各项。
