...
- [ ] **G8a 代码块检测(确定性,先行)**:born-digital 的代码块由**等宽字体名 + 缩进/行结构**确定性判定——IR 加 `Element::CodeBlock`(或 chunk kind),Markdown 输出 fenced code。无模型、零依赖,直接补 Docling 的 code 标注能力大半场景。
- [ ] **G8b N3b HTTP enhancer(全能力速通)**:docling-serve 兼容的 HTTP 后端经 `Enhancer` 边界接入——**一次接入即获得全部五项**(公式/代码/图片分类与描述/图表/整页 VLM),由后端模型负责,结果归一回 IR 带 `source: "http:<backend>"`。**依赖征询:HTTP 客户端(`ureq` 倾向,同步轻量)**。这是五项能力的兜底与上限,先于内嵌做。
- [ ] **G8c 选择性 ONNX 内嵌(spike 门控,P4 模式)**:
  ...
  - [ ] **G8d 整页 VLM(远期 spike)**:SmolDocling/SmolVLM 级(~256M 参数)在纯 Rust 运行时(`candle`)的可行性 spike——能跑则提供可选 `docparse-vlm` crate(模型外部文件,CPU 慢速可接受按页触发);不能则此能力长驻 G8b HTTP。**依赖征询:candle**。
- **验收**:G8a 代码块在 `code_and_formula` 样例正确标注;G8b 对接一个真实后端端到端(图片描述/公式至少各一例),数字 clean 页零外呼;G8c/G8d 各以 spike 结论文档定去留。
...
flowchart LR
  ...
  G8a[G8a 代码块·确定性] 
  G8b[G8b HTTP 外接<br/>解锁五项增强] --> G8c[G8c 选择性内嵌 spike] --> G8d[G8d 整页 VLM spike]
...
| G8 语义增强面 | 五项能力(用户点名) | G8b 低险先行;G8c 有身份决策点;G8d 风险高 | ureq(G8b)/candle(G8d) |
