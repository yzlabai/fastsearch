...
- [ ] **G9b · 列表建模**:IR 新增列表语义(chunk kind `list_item` + 层级);确定性检测(项目符号/编号前缀 + 缩进层级,参 wcag-algs `ListLabelsDetection` 方法论独立实现);tagged 路径直接取 L/LI。Markdown 输出 `- ` / `1. `。
  - 验收:含嵌套列表的样例(docling 测试集 `multi_page` 等含列表)结构正确;MHS/NID 零回归。
- [ ] **G9c · 标题分级**:现状只有"是/不是"一档。字号聚类 → 层级(页面字号分布的离散档位映射 H1–H3);chunk 的 `heading_path` 已按字号嵌套,正式化为 level 字段;Markdown 输出 `##`/`###`/`####`。
  - 验收:`2203`(1./4.1. 两级)层级正确;MHS 预期回升(当前 0.597 部分源于平层)。
- [ ] **G9d · 表格检出覆盖**(谨慎推进):差距 2203 7vs13、2305 2vs12、amt 0vs4。在 P1c"放宽门必回归"教训约束下:逐文档分析 ODL 多检出的表型(非数值/两列/键值表),只对**有结构校验支撑**的形态开门;每步带记分牌回归门。无把握的形态留给 VLM(`--vlm-tables`)。
- [ ] 零头:`right_to_left_03` 诊断(rtl_01/02 已 0.97);图片导出接口(对齐 ODL `--image-output`,我方已抽图,缺导出面)。
- **验收总门**:vs ODL MHS 0.597 → ≥0.65,TEDS 0.128 → ≥0.2,clean 文档零回归。
