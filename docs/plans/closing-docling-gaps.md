...
- [x] **本地语料无 panic 压测 ✅**(2026-06-11,[testresults](../testresults/2026-06-11-g7-stress.md)):`scripts/stress.py`(可重跑)——647 份清洁语料(8 格式,含 veraPDF 参考库 26 份边缘 PDF)+ 1200 个确定性变异样本(截断/字节翻转,SHA-256 播种可复现)= **1847 输入 0 panic 0 超时**;唯一干净 error 是 Shift-JIS CSV(编码探测留 TODO)。吞吐参考:12MB/32 页杂志 0.10s,647 份 2.9s。
- [ ] arXiv 千份级原版口径(需网络/存储决策)与 **cargo-fuzz** 覆盖引导模糊(**征询 dev 依赖 cargo-fuzz**,需 nightly)——变异压测已覆盖字节级破坏,fuzz 价值在覆盖引导的深路径。
