...
### M7 · 质量评分驱动的路由 + 外接 AI — *模块 7、8* ✅ 完成（2026-06-09）
**"多数页不碰模型、只难例升级"的成本论点落地**。

- [x] 质量评分判难例：`quality::assess_page`（无文本=扫描、高乱码）。
- [x] **可插拔外接边界**：`core::enhance` `Capability`（版本化）+ `Enhancer` trait + `plan`/`apply`，按页触发、归一回同一 IR、低 confidence 标记。
- [x] 主流程**无外接独立产出**（单测钉死）；CLI `--route-plan`。
- **验收 ✅**：1901 0/15、2408 0/14 硬页（数字页零模型），chinese_scan 1/1 升级；StubOcr 单测证明数字页不动/扫描页替换；41 单测；零回归；clippy 零 warning。devlog：[2026-06-09-m7-routing-enhance.md](../devlogs/2026-06-09-m7-routing-enhance.md)。
- **遗留**：不内置真实 OCR/LLM（边界+演示）；元素级 source 标签；多 enhancer 竞价；安全预检（模块 9）。
