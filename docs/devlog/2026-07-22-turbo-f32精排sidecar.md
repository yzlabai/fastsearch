...
- **修 reload 泄漏（实为 bug 非纯 polish）**：Step 1 的 `load` 把 `free` 清空、`next_slot=max+1` → reload 后
  删除留下的洞**永不复用**，churn 型（删+增）工作负载文件随**总 churn** 无界增长（非活集大小）。修：
  `slot_state()` 由 entries 重建 `(next, free=[0,next) 未占的洞)`，`load`/`ensure_sidecar` 都用它 → 洞被复用、
  文件不超 `max-ever-concurrent-live·stride`。测 `rerank_reload_reclaims_holes`（删 4 + reload + 增 4 → 文件 10
  slot 而非 14）。
- **`compact()` 永久缩量**：把活条目 f32 重写成紧凑连续 slot（读旧 slot → 写临时紧凑文件 → 原子 rename → 重开 +
  重指 entry.slot），文件缩到 `live·stride`。churn 由 `free` 复用已控增长；`compact` 处理**永久删除后**的磁盘回收
  （参照 HNSW `compact`；本档手动，自动触发留后续）。测 `rerank_compact_shrinks`（删半数 → 12→6 slot、检索语义
  不变、无脏读）。**替代**：全 rebuild-from-PG-truth 亦回收（不变量 #2），`compact` 是不重灌的轻量路径。
- **mmap/`read_at` 未做**：候选批小、`Mutex<File>` 无实测瓶颈——不为无证据的性能放宽（避 Speculative；需剖析或
  unsafe 决策再评估）。
- **收口 workspace 322 绿**（vector +2）。

## 下一步

- 全部 Step 完成。可选未来：`compact` 自动触发（洞过半，仿 HNSW `maybe_compact`）；mmap/`read_at`（需剖析/unsafe）。
