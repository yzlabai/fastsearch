...
- **Step 2 接引擎/server**：`VectorBackendKind::TurboQuant{bits, rerank_oversample}` + `VectorStore` 传
  sidecar 路径（`sidecar_path(data_dir/vector.bin)`）+ engine `open_with` + server env `FASTSEARCH_TURBO_RERANK`。
