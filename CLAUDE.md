...
Cargo workspace，十三个 crate（core/pdf/docx/html/ocr/raster/vlm/xlsx/pptx/md/csv/srt/cli）。**`core` 不依赖任何 PDF 库**——阅读顺序与输出对所有格式通用，加格式只需实现 `DocumentParser` trait 并在 CLI 注册表加一行。
...
战略见 [docs/roadmap.md](docs/roadmap.md)；近期里程碑 N1–N6 见 [docs/plans/next-iteration.md](docs/plans/next-iteration.md)（M1–M7 历史见 [docs/plans/beating-docling.md](docs/plans/beating-docling.md)）。**已完成**（2026-06-10，见 docs/devlogs/）：M1–M7 + N1–N5 全部（评测/服务化/OCR enhancer/表格/安全）+ Phase 4 主体（G1a/G2/G4/G8a/G8b 首增量/G9 全部）。**记分牌**（born-digital LTR）：vs ODL NID 0.792/MHS 0.685/TEDS 0.419；vs Docling NID 0.822/MHS 0.643/TEDS 0.474。G9d（表结构：通道列+规则线分带+二分重试,验收门 TEDS≥0.2 过）见 devlogs/2026-06-10-g9d-ruled-table-structure。**剩余池**：G1b 余项（EML/图片即文档，候依赖征询）、G7 加强项（arXiv 千份/cargo-fuzz，候决策）、G8b 真实服务 e2e（候 Ollama）、`--vlm-tables`；详见 [docs/plans/closing-docling-gaps.md](docs/plans/closing-docling-gaps.md)。⚠️ 经验：记分牌大跳几乎全是评测/输出管线 bug——分数可疑先怀疑管线（G9d 的行对齐 DP 又一例：尺子不修,结构改进不可见）。
