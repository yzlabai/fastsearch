...
Cargo workspace，三个 crate。**`core` 不依赖任何 PDF 库**——阅读顺序与输出对所有格式通用，加格式只需实现 `DocumentParser` trait 并在 CLI 注册表加一行。
...
战略见 [docs/roadmap.md](docs/roadmap.md)；近期里程碑 N1–N6 见 [docs/plans/next-iteration.md](docs/plans/next-iteration.md)（M1–M7 历史见 [docs/plans/beating-docling.md](docs/plans/beating-docling.md)）。**已完成**（2026-06-10，见 docs/devlogs/）：M1–M7 全部 + N1 评测（vs ODL NID 0.764/MHS 0.627、vs Docling NID 0.833/MHS 0.645，clean LTR 达同台水平）+ N2 服务化（MCP stdio + REST，四接口逐字节一致）+ N4 大部（表格四检测器）+ N5 安全预检（隐藏文本过滤 + zip-bomb/页数守卫）。**近期仅剩 N3 真实 enhancer**（部署选型待用户决策，已暂缓）；其余剩项属确定性天花板（CJK 版面/无框表 recall，P1c 已证放宽门必回归）或远期（P4 ONNX、人工真值集）。⚠️ 经验：记分牌大跳几乎全是评测/输出管线 bug——分数可疑先怀疑管线（详见 devlogs/2026-06-10-session-summary）。
