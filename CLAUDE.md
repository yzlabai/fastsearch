...
战略见 [docs/roadmap.md](docs/roadmap.md)；执行里程碑 M1–M7 见 [docs/plans/beating-docling.md](docs/plans/beating-docling.md)；战术清单见 [docs/iteration-guide.md §5](docs/iteration-guide.md)。**近期执行层 M1–M7 全部完成**（2026-06-09，见 docs/devlogs/）：文本保真、IR 脊梁（版本化+provenance+quality）、版面可读、有框表格、多格式（PDF/DOCX/HTML）、RAG 切块+chunk↔bbox 引用、质量路由+外接边界。后续为远期（安全预检、服务化、ONNX、真实 enhancer、评测集回填）。
