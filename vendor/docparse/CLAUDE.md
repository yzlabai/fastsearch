...
| **换识别后端**（区域图→文本） | [core/region_reader.rs](crates/docparse-core/src/region_reader.rs) 的 `RegionReader` 是唯一接缝：`UniRec`(进程内 tract, [ocr/unirec.rs](crates/docparse-ocr/src/unirec.rs)) 与 `VlmRegionReader`(HTTP, [vlm/region.rs](crates/docparse-vlm/src/region.rs)) 互为实现;`table_model`/`transcribe` 两处编排对二者一视同仁。**并发归后端**(`read_batch` 默认串行,网络后端覆写为有界并发)。加新后端=加一个 impl,不动编排 |
| 加"能力"给四张脸 | CLI flag 在 `main.rs::Cli`;**服务面另需** `EnhanceOpts` 加字段 + `any_pdf_only()` + `validate()`(CLI 的 clap `conflicts_with` 在 MCP/REST 表达不了,约束靠它跨面统一) + [mcp.rs](crates/docparse-cli/src/mcp.rs) schema + [server.rs](crates/docparse-cli/src/server.rs) query。**只加 CLI 就破了"四接口一份输出"** |
