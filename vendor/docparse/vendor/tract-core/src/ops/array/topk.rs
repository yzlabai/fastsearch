...
impl EvalOp for Topk {
    ...
    fn eval(&self, inputs: TVec<TValue>) -> TractResult<TVec<TValue>> {
        ...
        // docparse PATCH (vendor/PATCHES.md #2): tract-onnx maps ONNX
        // Cast(to=INT64) to TDim, so int64 *data* (e.g. a 0/1 mask) can reach
        // TopK as TDim, which `dispatch_numbers!` rejects ("TDim is not a
        // number"). TDim at eval holds concrete ints — sort as i64, then
        // restore TDim on the values output so the declared fact still matches.
